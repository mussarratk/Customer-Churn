...
https://www.coursera.org/learn/coding-and-automation-for-data-analysis-with-generative-ai/lecture/5hOhv/real-world-applications-of-generative-ai-in-data-analysis

- predicting customer churn, generative AI can automatically create new features like time since last purchase or average basket size. These features are often more predictive of customer behavior than the raw data itself.
- AI can automate model selection and hyperparameter tuning. This is essential when dealing with large data sets. Once you've built your model, evaluating its performance is key. In a retail scenario, for instance, you'd want to know if your churn prediction model is actually identifying at-risk customers accurately. AI can calculate key metrics like accuracy, precision, recall, and F1 score without you having to manually crunch the numbers. This means you get real-time feedback on how well the model is performing, allowing you to make adjustments quickly if needed. Generative AI helps explain which features are driving the predictions. For example, it might show that customers who haven't interacted with a brand in over a month are more likely to churn, or that customers who tend to purchase only during sales events are at a higher risk of leaving. Once the model is ready and understood, deployment becomes the next real-world challenge. In e-commerce, for example, you need the model to continuously analyze new data, such as when customers make a purchase or contact support, to keep predictions up to date. Generative AI automates the deployment process and model monitoring, making sure that the model is integrated with the company's data systems and continuously refreshed with the latest data. Throughout the entire workflow, from data cleaning to model deployment, generative AI amplifies the efficiency and accuracy of your analysis. 
